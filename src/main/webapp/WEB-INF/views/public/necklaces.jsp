...
<div class="normal-box">
  <c:if test="${not empty category}">
    <c:choose>
      ...
      <c:when test="${category == 'NECKLACE'}"><c:import url="../fragments/banners/necklacesBanner.jsp"/></c:when>
...
<div class="small-box">
  <div class="container-fluid"
       style="margin-top:-20px;padding:0px;min-height:500px;height:100vh;background-position:center;background-repeat:no-repeat;background-size:cover;width:100%;position:absolute;">
    <div class="necklace-header">
      <div style="text-align:center">
        <h3>Available on desktop</h3>
        <h1 style="font-family: 'Vollkorn', serif;font-size:48px;font-style:italic;">Necklaces</h1><br>
        <a href="/feed" class="btn btn_big" style="width:inherit; font-size:16px;">Keep Shopping</a>
      </div>
    </div>
    <div class="row items"></div>
  </div>
  ...
  <script>
    ...
    function positionCards(selector) {
      ...
      $(selector).each(function (i) {
        if ($(window).width() > 468 || ($(window).width() <= 468 && $(this).hasClass('hidemob') == false)) {
          itemsProcessed += 1;
  ...
  <script>
    ...
    function saveButtonClick(itemId, location) {
